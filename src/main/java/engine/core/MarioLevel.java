...
import main.java.engine.graphics.MarioTilemap;
...
public class MarioLevel {
            ...
            for (int x = 0; x < lines[y].length(); x++) {
                ...
                switch (c) {
                    ...
                    case 't':
                        ...
                        if (
                            x < lines[y].length() - 1 &&
                            Character.toLowerCase(lines[y].charAt(x + 1)) != 't' &&
                            x > 0 &&
                            Character.toLowerCase(lines[y].charAt(x - 1)) != 't'
                        ) {
                            singlePipe = true;
                        ...
                        if (
                            x > 0 &&
                            (this.levelTiles[x - 1][y] == 18 || this.levelTiles[x - 1][y] == 20)
                        ) {
                            tempIndex += 1;
                        }
    ...
    public void render(Graphics og, int cameraX, int cameraY) {
        ...
        if (cameraX + MarioGame.width >= this.exitTileX * 16) {
            this.flag.render(
                og,
                this.exitTileX * 16 - 8 - cameraX,
                Math.max(1, this.exitTileY - 11) * 16 + 16 - cameraY
            );
        }
    }
}
