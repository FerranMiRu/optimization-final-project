...
import main.java.engine.effects.*;
import main.java.engine.graphics.MarioBackground;
...
import main.java.engine.helper.TileFeature;
import main.java.engine.sprites.*;
...
public class MarioWorld {
    ...
    public void initializeLevel(String level, int timer) {
        ...
        this.mario = new Mario(
            this.visuals,
            this.level.marioTileX * 16,
            this.level.marioTileY * 16
        );
    ...
    public void addEvent(EventType eventType, int eventParam) {
        ...
        this.lastFrameEvents.add(
            new MarioEvent(eventType, eventParam, mario.x, mario.y, marioState, this.currentTick)
        );
    }
    ...
    public int[][] getSceneObservation(float centerX, float centerY, int detail) {
        ...
        for (
            int y = centerYInMap - MarioGame.tileHeight / 2, obsY = 0;
            y < centerYInMap + MarioGame.tileHeight / 2;
            y++, obsY++
        ) {
            for (
                int x = centerXInMap - MarioGame.tileWidth / 2, obsX = 0;
                x < centerXInMap + MarioGame.tileWidth / 2;
                x++, obsX++
            ) {
                int currentX = x;
                if (currentX < 0) {
                ...
                ret[obsX][obsY] = MarioForwardModel.getBlockValueGeneralization(
                    this.level.getBlock(currentX, currentY),
                    detail
                );
            }
        }
    ...
    public int[][] getEnemiesObservation(float centerX, float centerY, int detail) {
        ...
        for (int w = 0; w < ret.length; w++) for (int h = 0; h < ret[0].length; h++) ret[w][h] = 0;

        for (MarioSprite sprite : sprites) {
            if (sprite.type == SpriteType.MARIO) continue;
            if (
                sprite.getMapX() >= 0 &&
                ...
                ret[obsX][obsY] = MarioForwardModel.getSpriteTypeGeneralization(
                    sprite.type,
                    detail
                );
            }
    ...
    public int[][] getMergedObservation(
        float centerX,
        float centerY,
        int sceneDetail,
        int enemiesDetail
    ...
    private boolean isEnemy(MarioSprite sprite) {
        return (
            sprite instanceof Enemy || sprite instanceof FlowerEnemy || sprite instanceof BulletBill
        );
    }
    ...
    public void update(boolean[] actions) {
        ...
        for (MarioSprite sprite : sprites) {
            if (
                sprite.x < cameraX - 64 ||
                sprite.x > cameraX + MarioGame.width + 64 ||
        ...
        for (int x = (int) cameraX / 16 - 1; x <= (int) (cameraX + MarioGame.width) / 16 + 1; x++) {
            for (
                int y = (int) cameraY / 16 - 1;
                y <= (int) (cameraY + MarioGame.height) / 16 + 1;
                y++
            ) {
                int dir = 0;
                ...
                if (dir != 0) {
                    ArrayList<TileFeature> features = TileFeature.getTileType(
                        this.level.getBlock(x, y)
                    );
                    if (features.contains(TileFeature.SPAWNER)) {
                        if (this.currentTick % 100 == 0) {
                            addSprite(
                                new BulletBill(this.visuals, x * 16 + 8 + dir * 8, y * 16 + 15, dir)
                            );
                        }
                    }
                }
    ...
    public void render(Graphics og) {
        ...
        for (MarioSprite sprite : sprites) {
            if (
                sprite.type == SpriteType.MUSHROOM ||
