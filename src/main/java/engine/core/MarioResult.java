...
public class MarioResult {
    ...
    public MarioResult(
        ...
        ArrayList<MarioAgentEvent> agentEvents
    ) {
        this.world = world;
    ...
    /**
     * Get the score given the equation from the Championship
     *
     * score = distance_traveled + 64 * devoured_flowers + 58 * devoured_mushrooms + 58 *
     *         * devoured_green_mushrooms + 42 * enemies_killed + 12 * enemies_killed_by_jump + 17 *
     *         * enemies_killed_by_shell + 4 * enemies_killed_by_fireball + 1024 * won_or_lost +
     *         + 32 * final_mario_state + 20 * coins_collected + 8 * time_left
     *
     * @return the score
     */
    public float getScore() {
        ...
        score += 17 * this.getKillsByShell();
        score += 4 * this.getKillsByFire();
        score += 1024 * (this.getGameStatus() == GameStatus.WIN ? 1 : 0);
        score += 32 * this.getMarioMode();
        score += 20 * this.getCurrentCoins();
        score +=
            (8 * (this.getRemainingTime() / 1000)) * // the time is in ms
            (this.getGameStatus() == GameStatus.WIN ? 1 : 0);
     ...
     * @return value of the x pixel that mario finished in
     */
    ...
    public int getKillsTotal() {
        ...
        for (MarioEvent e : this.gameEvents) {
            if (
                e.getEventType() == EventType.STOMP_KILL.getValue() ||
    ...
    public int getNumBumpQuestionBlock() {
        ...
        for (MarioEvent e : this.gameEvents) {
            if (
                e.getEventType() == EventType.BUMP.getValue() &&
    ...
    public int getNumBumpBrick() {
        ...
        for (MarioEvent e : this.gameEvents) {
            if (
                e.getEventType() == EventType.BUMP.getValue() &&
                e.getEventParam() == MarioForwardModel.OBS_BRICK
            ) {
                bump += 1;
            }
        }
    ...
    public int getNumCollectedMushrooms() {
        ...
        for (MarioEvent e : this.gameEvents) {
            if (
                e.getEventType() == EventType.COLLECT.getValue() &&
    ...
    /**
     * get the number of life mushroom collected by mario
     *
     * @return the number of collected life mushrooms by mario
     */
    public int getNumCollectedLifeMushrooms() {
        int collect = 0;
        for (MarioEvent e : this.gameEvents) {
            if (
                e.getEventType() == EventType.COLLECT.getValue() &&
    ...
    public int getNumCollectedTileCoins() {
        ...
        for (MarioEvent e : this.gameEvents) {
            if (
                e.getEventType() == EventType.COLLECT.getValue() &&
                e.getEventParam() == MarioForwardModel.OBS_COIN
            ) {
                collect += 1;
            }
        }
