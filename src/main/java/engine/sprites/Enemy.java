package main.java.engine.sprites;

import java.awt.Graphics;
import main.java.engine.core.MarioSprite;
import main.java.engine.effects.DeathEffect;
import main.java.engine.effects.SquishEffect;
import main.java.engine.graphics.MarioImage;
import main.java.engine.helper.Assets;
import main.java.engine.helper.EventType;
import main.java.engine.helper.SpriteType;
...
public class Enemy extends MarioSprite {
    ...
    public Enemy(boolean visuals, float x, float y, int dir, SpriteType type) {
        ...
        this.avoidCliffs =
            this.type == SpriteType.RED_KOOPA || this.type == SpriteType.RED_KOOPA_WINGED;
        this.noFireballDeath =
            this.type == SpriteType.SPIKY || this.type == SpriteType.SPIKY_WINGED;
        this.facing = dir;
        if (this.facing == 0) {
            this.facing = 1;
        }
    ...
    public void collideCheck() {
        ...
        if (xMarioD > -width * 2 - 4 && xMarioD < width * 2 + 4) {
            if (yMarioD > -height && yMarioD < world.mario.height) {
                if (
                    type != SpriteType.SPIKY &&
                    type != SpriteType.SPIKY_WINGED &&
                    type != SpriteType.ENEMY_FLOWER &&
                    world.mario.ya > 0 &&
                    yMarioD <= 0 &&
                    ...
                    if (winged) {
                        ...
                        if (
                            type == SpriteType.GREEN_KOOPA || type == SpriteType.GREEN_KOOPA_WINGED
                        ) {
                            this.world.addSprite(
                                new Shell(this.graphics != null, x, y, 1, this.initialCode)
                            );
                        } else if (
                            type == SpriteType.RED_KOOPA || type == SpriteType.RED_KOOPA_WINGED
                        ) {
                            this.world.addSprite(
                                new Shell(this.graphics != null, x, y, 0, this.initialCode)
                            );
                ...
                if (this.graphics != null) {
                    if (
                        ...
                        this.world.addEffect(
                            new DeathEffect(this.x, this.y, this.graphics.flipX, 42, -5)
                        );
                    } else if (
                        this.type == SpriteType.RED_KOOPA ||
                        this.type == SpriteType.RED_KOOPA_WINGED
                        ...
                        this.world.addEffect(
                            new DeathEffect(this.x, this.y, this.graphics.flipX, 41, -5)
                        );
                    } else if (
                        this.type == SpriteType.GOOMBA || this.type == SpriteType.GOOMBA_WINGED
                    ) {
                        this.world.addEffect(
                            new DeathEffect(this.x, this.y, this.graphics.flipX, 44, -5)
                        );
                    } else if (
                        this.type == SpriteType.SPIKY || this.type == SpriteType.SPIKY_WINGED
                    ) {
                        this.world.addEffect(
                            new DeathEffect(this.x, this.y, this.graphics.flipX, 45, -5)
                        );
                    }
                }
                this.world.removeSprite(this);
    ...
    public void bumpCheck(int xTile, int yTile) {
        ...
        if (
            x + width > xTile * 16 && x - width < xTile * 16 + 16 && yTile == (int) ((y - 1) / 16)
        ) {
    ...
    public void render(Graphics og) {
        if (winged) {
            if (
                type != SpriteType.RED_KOOPA &&
                type != SpriteType.GREEN_KOOPA &&
                type != SpriteType.RED_KOOPA_WINGED &&
                type != SpriteType.GREEN_KOOPA_WINGED
            ) {
                this.wingGraphics.flipX = false;
                this.wingGraphics.render(
                    og,
                    (int) (this.x - this.world.cameraX - 6),
                    (int) (this.y - this.world.cameraY - 6)
                );
                this.wingGraphics.flipX = true;
                this.wingGraphics.render(
                    og,
                    (int) (this.x - this.world.cameraX + 22),
                    (int) (this.y - this.world.cameraY - 6)
                );
            }
        }
        ...
        this.graphics.render(
            og,
            (int) (this.x - this.world.cameraX),
            (int) (this.y - this.world.cameraY)
        );
        ...
        if (winged) {
            if (
                ...
                this.wingGraphics.render(
                    og,
                    (int) (this.x - this.world.cameraX + shiftX),
                    (int) (this.y - this.world.cameraY - 8)
                );
            }
        }
    }
