...
public class Mario extends MarioSprite {
    ...
    private boolean isBlocking(float _x, float _y, float xa, float ya) {
        int xTile = (int) (_x / 16);
        int yTile = (int) (_y / 16);
        if (xTile == (int) (this.x / 16) && yTile == (int) (this.y / 16)) return false;
    ...
    public void updateGraphics() {
        ...
        if (currentLarge) {
            ...
            if (Math.abs(xa) > 10) frameIndex += 3;
            if (!onGround) {
                if (Math.abs(xa) > 10) frameIndex = 6;
                else frameIndex = 5;
            }
        } else {
            frameIndex = ((int) (marioFrameSpeed / 20)) % 2;
            if (Math.abs(xa) > 10) frameIndex += 2;
            if (!onGround) {
                if (Math.abs(xa) > 10) frameIndex = 5;
                else frameIndex = 4;
            }
        }

        if (onGround && ((facing == -1 && xa > 0) || (facing == 1 && xa < 0))) {
            if (xa > 1 || xa < -1) frameIndex = currentLarge ? 8 : 7;
        }
    ...
    public void update() {
        ...
        if (
            actions[MarioActions.SPEED.getValue()] &&
            canShoot &&
            isFire &&
    ...
    public void getHurt() {
        if (invulnerableTime > 0 || !this.alive) return;
