package main.java.agents.trondEllingsen;
...
public class Agent implements MarioAgent {
    ...
    private boolean enemyInRange(MarioForwardModel e, Rectangle r) {
        for (int i = 0; i < e.getEnemiesFloatPos().length; i += 3) {
            if (
                r.contains(
                    e.getEnemiesFloatPos()[i + 1] - e.getMarioFloatPos()[0],
    ...
    public boolean[] getActions(MarioForwardModel model, MarioTimer timer) {
        ...
        final boolean dangerOfGap = dangerOfGap(
            model.getMarioScreenTilePos()[0],
            model.getMarioScreenTilePos()[1],
        ...
        if ((model.isMarioOnGround() || model.mayMarioJump()) && !jumpType.equals(JumpType.NONE)) {
            ...
            final int wallHeight = getWallHeight(
                model.getMarioScreenTilePos()[0],
                model.getMarioScreenTilePos()[1],
                model.getScreenSceneObservation()
            );
        ...
        final boolean isFalling =
            prevY < model.getMarioFloatPos()[1] && jumpType.equals(JumpType.NONE);
        action[MarioActions.LEFT.getValue()] =
            isFalling && ((dangerOfEnemy && dangerOfEnemyAbove) || dangerOfGap);
        action[MarioActions.RIGHT.getValue()] =
            !isFalling && !(dangerOfEnemyAbove && jumpType == JumpType.WALL);
        action[MarioActions.JUMP.getValue()] =
            !jumpType.equals(JumpType.NONE) && jumpCount < jumpSize;
        action[MarioActions.SPEED.getValue()] = !(jumpType.equals(JumpType.ENEMY) &&
            action[MarioActions.SPEED.getValue()] &&
            model.getMarioMode() == 2);
