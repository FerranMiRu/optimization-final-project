...
public final class MarioState extends SpriteState {
    ...
    public float xJumpSpeed = 0,
        yJumpSpeed = 0; // can we get rid of this somehow?
    ...
    public void print() {
        System.out.printf(
            "g=%d a:%d x:(%f,%f) v:(%f,%f) %s%s%s cost=%f\n",
            (int) g,
            action,
            x,
            y,
    ...
    public void move(int action) {
        ...
        if ((action & ACT_JUMP) != 0 || (jumpTime < 0 && !onGround && !sliding)) {
            if (jumpTime < 0) {
                ...
                // post-peak downward jump trajectory
        ...
        if (
            ((action & ACT_LEFT) == 0 && (action & ACT_RIGHT) == 0) || ducking || ya < 0 || onGround
        ) {
            sliding = false;
        }
    ...
    private boolean isBlocking(float _x, float _y, float xa, float ya) {
        ...
        if (blocking && ya < 0) ws = ws.bump(x, y, big);
