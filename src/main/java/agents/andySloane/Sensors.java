...
public class Sensors {
    ...
    public void updateReadings(MarioForwardModel model) {
        ...
        fireballsOnScreen = 0;
        for (int x = 0; x < levelScene.length; ++x) for (
            int y = 0;
            y < levelScene[0].length;
            ++y
        ) asciiScene[x][y] = asciiLevel(levelScene[x][y]);
        for (int x = 0; x < enemiesScene.length; ++x) for (
            int y = 0;
            y < enemiesScene[0].length;
            ++y
        ) {
            int enemy = enemiesScene[x][y];
            if (enemy == MarioForwardModel.OBS_NONE) continue;
            if (enemy == MarioForwardModel.OBS_FIREBALL) fireballsOnScreen++;
    ...
    public String toString() {
        ...
        for (String[] sceneRow : asciiScene) {
            for (String square : sceneRow) sb.append(square + " ");
            sb.append('\n');
        }
        return sb.toString();
    }
    ...
    private String asciiEnemy(int enemySquare) {
        ...
        if (
            enemySquare == MarioForwardModel.OBS_RED_KOOPA ||
            enemySquare == MarioForwardModel.OBS_GREEN_KOOPA
        ) {
            return "n";
        }
        if (
            enemySquare == MarioForwardModel.OBS_GOOMBA_WINGED ||
            enemySquare == MarioForwardModel.OBS_RED_KOOPA_WINGED ||
