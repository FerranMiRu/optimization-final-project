...
public final class WorldState {
    ...
    void buildHeightMap() {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            for (
                j = 15;
                j >= 0;
                j-- // find the first block from the bottom
            ) if (map[j][i] != 0) break;
            if (j < 0 || j + MapY < 8) {
                // this is probably a ceiling
                heightmap[i] = 22;
            } else {
                for (; j >= 0; j--) if (map[j][i] == 0 || map[j][i] == 1) break; // 1 is mario, 0 is blank
                heightmap[i] = j + 1;
            }
    ...
    void _removeTile(WSHashKey h, int x, int y) {
        ...
        for (int j = 0; j < 16; j++) for (int i = 0; i < 16; i++) newmap[i][j] = map[i][j];
        newmap[x][y] = 0;
        map = newmap;
    }
    ...
    public void syncEnemies(WorldState prevws, float[] enemyObs, MarioState ms) {
        ...
        for (int i = 0; i < enemyObs.length; i += 3) obs[i / 3] = new EnemyObservation(
            (int) enemyObs[i],
            enemyObs[i + 1],
            enemyObs[i + 2]
        );
        ...
        for (EnemyObservation eobs : obs) {
            ...
            if (closest == null || closestdist > 64) {
                // allow a slop of 8 pixels
                // assume new enemy
            ...
            if (closest != null) newenemies.add(closest);
        }
        enemies = newenemies;
    }
    ...
    public WorldState interact(MarioState ms, boolean verbose) {
        ...
        for (SpriteState s : ws.addqueue) {
            if (verbose) {
                System.out.printf(
                    "interact: new e t=%d xy=%f,%f xaya=%f,%f deadTime=%d\n",
                    s.type,
                    s.x,
                    s.y,
                    s.xa,
    ...
    WorldState removeTile(int x, int y) {
        x -= MapX;
        y -= MapY;
        if (x < 0 || x >= 16 || y < 0 || y >= 16) return this;
