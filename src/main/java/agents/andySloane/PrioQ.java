...
public class PrioQ {
    ...
    public void drop() {
        ...
        int l = size >> 1;
        for (int i = 0; i < l; ++i) queue[--size] = null;
        for (int i = (size >>> 1) - 1; i >= 0; i--) siftDown(i, queue[i]);
    }
    ...
    private void siftDown(int k, MarioState x) {
        ...
        while (k < half) {
            ...
            int right = child + 1;
            if (right < size && compare(c, queue[right]) > 0) c = queue[child = right];
