...
public abstract class HeuristicSearchingAgent implements MarioAgent {
    ...
    protected final float cost(MarioState s, MarioState initial) {
        ...
        if (s.dead) return Tunables.DeadCost;
        ...
        if (
            MarioX < 0 || MarioX >= 22 // mario ran off the screen; we're done
        ...
        if (s.onGround) {
            if (s.y <= fLedgeY) return damage + xsteps;
            else {
            ...
            if (
                r.dead && l.dead // we're dead no matter what! forget it!
            ...
            if (MarioMath.canReachLedge(s.x, s.xa, apogeesteps, apogeey, fLedgeX, fLedgeY)) {
                ...
                for (float i = l.x; i <= r.x; i += 16) {
                    ...
                    if (s.ws.heightmap[idx] < perchY && s.ws.heightmap[idx] >= MarioY) {
                        // look for leftmost edge
    ...
    public static final boolean useless_action(int a, MarioState s) {
        ...
        int ix = (int) s.x;
        if (
            (a & MarioState.ACT_RIGHT) > 0 && s.xa == 0 && (s.x - ix) == 0 && (ix & 15) == (16 - 5)
        ) {
            // ok, we are exactly at a brick boundary. is there something there?
            return (
                s.ws.isBlocking((ix + 5) / 16, (int) (s.y / 16), 1, s.ya) ||
    ...
    public boolean[] getActions(MarioForwardModel model, MarioTimer timer) {
        if (
            won // we won! we can't do anything!
        ) return action;
    ...
    private void resync(MarioForwardModel model, boolean x, boolean y) {
        ...
        if (ms_prev != null) {
            ...
            if (x) ms.xa = (ms.x - ms_prev.x) * 0.89f;
            if (y) ms.ya = (ms.y - ms_prev.y) * 0.85f;
        }
    }
