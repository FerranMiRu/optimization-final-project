...
public final class Agent extends HeuristicSearchingAgent {
    ...
    protected int searchForAction(MarioState initialState, WorldState ws) {
        ...
        // add initial set
        for (a = 0; a < 16; a++) {
            if (useless_action(a, initialState)) continue;
        ...
        for (n = 0; n < maxSteps && !pq.isEmpty(); n++) {
            ...
            for (a = 0; a < 16; a++) {
                if (useless_action(a, next)) continue;
                ...
                ms.cost =
                    ms.g + h + ((a & MarioState.ACT_JUMP) > 0 ? Tunables.FeetOnTheGroundBonus : 0);
                bestfound = marioMin(ms, bestfound);

                if (h < 0.1f) return ms.root_action;

                if (ms.cost < threshold) pq.offer(ms);
            }
        }

        if (!pq.isEmpty()) bestfound = marioMin(pq.poll(), bestfound);
        // return best so far
        pq.clear();
