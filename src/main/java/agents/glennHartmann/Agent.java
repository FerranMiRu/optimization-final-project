...
public class Agent implements MarioAgent {
    ...
    private byte[][] decode(MarioForwardModel model, int[][] state) {
        ...
        for (int i = 0; i < dstate.length; ++i) for (
            int j = 0;
            j < dstate[0].length;
            ++j
        ) dstate[i][j] = 2;
    ...
    public boolean[] getActions(MarioForwardModel model, MarioTimer timer) {
        ...
        // safe to. If there's danger of falling, jump no matter what
            ...
            (((dangerFromEnemies(enemiesFromBitmap) || block(levelSceneFromBitmap)) &&
                    safeToJump(levelSceneFromBitmap, enemiesFromBitmap)) ||
                dangerFromGaps(levelSceneFromBitmap)) &&
            model.mayMarioJump()
        ) {
