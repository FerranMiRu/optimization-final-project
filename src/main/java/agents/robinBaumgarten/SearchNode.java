...
public class SearchNode {
    ...
    public float calcRemainingTime(float marioX, float marioXA) {
        return (
            (100000 - (maxForwardMovement(marioXA, 1000) + marioX)) / Helper.maxMarioSpeed - 1000
        );
    }

    public float getRemainingTime() {
        if (remainingTime > 0) return remainingTime;
        else return remainingTimeEstimated;
    }
    ...
    public float estimateRemainingTimeChild(boolean[] action, int repetitions) {
        ...
        return calcRemainingTime(
            this.sceneSnapshot.getMarioFloatPos()[0] + childbehaviorDistanceAndSpeed[0],
    ...
    public SearchNode(boolean[] action, int repetitions, SearchNode parent) {
        ...
        if (parent != null) timeElapsed = parent.timeElapsed + repetitions;
        else timeElapsed = 0;
    }
    ...
    public float simulatePos() {
        ...
        remainingTime =
            calcRemainingTime(
                this.sceneSnapshot.getMarioFloatPos()[0],
                this.sceneSnapshot.getMarioFloatVelocity()[0]
            ) +
            marioDamage * (1000000 - 100 * distanceFromOrigin);
        if (isInVisitedList) remainingTime += Helper.visitedListPenalty;
