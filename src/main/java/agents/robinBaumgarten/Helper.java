...
public class Helper {
    ...
    public static String getActionString(boolean[] action) {
        String s = "";
        if (action[MarioActions.RIGHT.getValue()]) s += "Forward ";
        if (action[MarioActions.LEFT.getValue()]) s += "Backward ";
        if (action[MarioActions.SPEED.getValue()]) s += "Speed ";
        if (action[MarioActions.JUMP.getValue()]) s += "Jump ";
        if (action[MarioActions.DOWN.getValue()]) s += "Duck";
        if (s.length() == 0) {
            s = "[NONE]";
        }
        return s;
    ...
    public static float[] estimateMaximumForwardMovement(
        float currentAccel,
        boolean[] action,
        int ticks
    ) {
        ...
        int dir = 0;
        if (action[MarioActions.LEFT.getValue()]) dir = -1;
        if (action[MarioActions.RIGHT.getValue()]) dir = 1;
    ...
    public static boolean[] createAction(
        boolean left,
        boolean right,
        boolean down,
        boolean jump,
    ...
    public static boolean canJumpHigher(SearchNode node, boolean checkParent) {
        if (
            node.parentPos != null && checkParent && canJumpHigher(node.parentPos, false)
        ) return true;
    ...
    public static ArrayList<boolean[]> createPossibleActions(SearchNode node) {
        ...
        // jump
        if (canJumpHigher(node, true)) possibleActions.add(
            Helper.createAction(false, false, false, true, false)
        );
        if (canJumpHigher(node, true)) possibleActions.add(
            Helper.createAction(false, false, false, true, true)
        );

        // run right
        possibleActions.add(Helper.createAction(false, true, false, false, true));
        if (canJumpHigher(node, true)) possibleActions.add(
            Helper.createAction(false, true, false, true, true)
        );
        possibleActions.add(Helper.createAction(false, true, false, false, false));
        if (canJumpHigher(node, true)) possibleActions.add(
            Helper.createAction(false, true, false, true, false)
        );

        // run left
        possibleActions.add(Helper.createAction(true, false, false, false, false));
        if (canJumpHigher(node, true)) possibleActions.add(
            Helper.createAction(true, false, false, true, false)
        );
        possibleActions.add(Helper.createAction(true, false, false, false, true));
        if (canJumpHigher(node, true)) possibleActions.add(
            Helper.createAction(true, false, false, true, true)
        );
