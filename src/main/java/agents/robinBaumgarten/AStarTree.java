...
public class AStarTree {
    ...
    private MarioForwardModel search(MarioTimer timer) {
        ...
        while (
            posPool.size() != 0 &&
            ((bestPosition.sceneSnapshot.getMarioFloatPos()[0] - currentSearchStartingMarioXPos <
                    maxRight) ||
                !currentGood) &&
            ...
            if (realRemainingTime < 0) {
                ...
                !current.isInVisitedList &&
                isInVisited(
                    (int) current.sceneSnapshot.getMarioFloatPos()[0],
                    (int) current.sceneSnapshot.getMarioFloatPos()[1],
                    current.timeElapsed
                )
            ) {
            ...
            if (currentGood) {
                if (bestPosition.getRemainingTime() > current.getRemainingTime()) bestPosition =
                    current;
                if (
                    current.sceneSnapshot.getMarioFloatPos()[0] >
                    furthestPosition.sceneSnapshot.getMarioFloatPos()[0]
                ) furthestPosition = current;
            }
        }
        if (
            current.sceneSnapshot.getMarioFloatPos()[0] - currentSearchStartingMarioXPos <
                maxRight &&
            furthestPosition.sceneSnapshot.getMarioFloatPos()[0] >
            bestPosition.sceneSnapshot.getMarioFloatPos()[0] + 20 // Couldnt plan till end of screen, take furthest
    ...
    private ArrayList<boolean[]> extractPlan() {
        ...
        while (current.parentPos != null) {
            for (int i = 0; i < current.repetitions; i++) actions.add(0, current.action);
    ...
    public boolean[] optimise(MarioForwardModel model, MarioTimer timer) {
        ...
        if (currentActionPlan.size() > 0) action = currentActionPlan.remove(0);
        return action;
    }
