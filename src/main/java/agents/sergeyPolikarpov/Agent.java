...
public class Agent implements MarioAgent {
    ...
    private int[][] progressed_action = new int[][] {
    ...
    private double[][] detector_of_holes_buffer_of_inputs =
        new double[detector_of_holes_deep_of_buffer][detector_of_holes_numberOfInputs];
    ...
    private void detector_of_holes_push_inputs_to_buffer(double[] inputs) {
        for (int i = 0; i < detector_of_holes_buffer_of_inputs.length - 1; i++) {
            for (int j = 0; j < detector_of_holes_buffer_of_inputs[i].length; j++) {
                detector_of_holes_buffer_of_inputs[i][j] = detector_of_holes_buffer_of_inputs[i +
                1][j];
            }
        }
        for (int i = 0; i < inputs.length; i++) {
            detector_of_holes_buffer_of_inputs[detector_of_holes_buffer_of_inputs.length - 1][i] =
                inputs[i];
        }
    ...
    public void initialize(MarioForwardModel model, MarioTimer timer) {
        ...
        detector_of_holes = new CyberNeuron(
            detector_of_holes_numberOfInputs,
            detector_of_holes_numberOfOutputs
        );
    ...
    public boolean[] getActions(MarioForwardModel model, MarioTimer timer) {
        ...
        if (force_long_jump_forward == false) {
            if (
                ...
                if (agent_is_active == false) {
                    /************ if cyberneurons are inactive *****************/
            ...
            if (action_in_progress == true) {
                /************ do actions from selected sequence *****************/
                ...
                if (
                    count_of_action_in_progress >= progressed_action[action_in_progress_type].length
                ) {
                    action_in_progress = false;
                }
            }
            ...
            count_of_action_in_progress++;
            if (action_in_progress_type == 2) if (
                count_of_action_in_progress >= progressed_action[action_in_progress_type].length
            ) {
                count_of_action_in_progress = 0;
                action_in_progress_type = 0;
            ...
            if (action_in_progress_type == 0) if (
                count_of_action_in_progress >= progressed_action[action_in_progress_type].length
            ) {
                action_in_progress = false;
        ...
        if (forward_mean <= 0) if (forward_mean <= 0) {
        ...
        if (Mario_pos[1] > 251) {
            ...
            if (detector_of_holes_buffer_is_full) {
                double[] detector_of_holes_targeted_outs =
                    new double[detector_of_holes_numberOfOutputs];
