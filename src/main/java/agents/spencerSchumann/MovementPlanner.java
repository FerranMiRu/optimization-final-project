...
public class MovementPlanner {
    ...
    private int flightTimeForJump(int jumpTime, float height) {
        ...
        while (true) {
            sim.update(coast);
            if (sim.mario.onGround) return sim.getTicks();
            if (sim.mario.vy > 0.0f && sim.mario.y > height) return -1;
        }
    }
    ...
    private int ticksToPos(float pos) {
        ...
        while (true) {
            sim.update(run);
            if (sim.mario.x >= pos) return sim.getTicks();
        }
    }
    ...
    private boolean checkPlan(PlanRunner plan, Edge targetFloor) {
        ...
        plan.rewind();
        return (
            sim.mario.x > targetFloor.x1 - 4.0f &&
    ...
    private PlanRunner planJump(Edge currentFloor, Edge targetFloor) {
        ...
        if (xdiff < 0.0f) xdiff = targetFloor.x2 - mario.x - 3.0f;
        ...
        for (jumpTime = 0; jumpTime <= 7; jumpTime++) {
            ...
            if (checkPlan(plan, targetFloor)) return plan;
            else continue;
        }
        return null;
    }
    ...
    public PlanRunner planMovement() {
        ...
        if (!targetFloors.isEmpty()) {
            for (Edge target : targetFloors) {
                if (target == currentFloor) continue;
