...
public class MotionSimulator {
    ...
    private void handleHorizontalInput(boolean[] action) {
        ...
        if (action[MarioActions.LEFT.getValue()]) mario.vx -= xSpeed;
        if (action[MarioActions.RIGHT.getValue()]) mario.vx += xSpeed;
    }
    ...
    private void handleJumpInput(boolean[] action) {
        mario.vy *= 0.85f;
        if (!mario.onGround) mario.vy += 3.0f;
        ...
        if (!action[MarioActions.JUMP.getValue()]) {
            mario.jumpTime = 0;
            if (mario.onGround) mario.mayJump = true;
        }
    ...
    private void moveHorizontally() {
        if (Math.abs(mario.vx) < 0.5f) mario.vx = 0.0f;
        ...
        for (Edge e : scene.walls) {
            ...
            if (e.y1 <= mario.y && e.y2 >= mario.y - mario.height) {
