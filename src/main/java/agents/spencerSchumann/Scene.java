...
public class Scene {
    ...
    public Scene(MarioForwardModel model, int[][] scene) {
        ...
        originX =
            (float) Math.floor(marioPos[0] / 16.0f) * 16.0f - (model.obsGridWidth / 2) * 16.0f;
        originY =
            (float) Math.floor(marioPos[1] / 16.0f) * 16.0f - (model.obsGridHeight / 2) * 16.0f;
    ...
    private void expandLedge(int[][] scene, boolean[][] visited, int x, int y) {
        ...
        floors.add(
            new Edge(
                originX + startx * 16.0f,
                originY + y * 16.0f,
                originX + (endx + 1) * 16.0f,
                originY + y * 16.0f
            )
        );
    }
