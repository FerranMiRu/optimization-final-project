package main.java.levelGenerators.linear;

import java.util.ArrayList;
import java.util.Random;
import main.java.engine.core.MarioLevelGenerator;
import main.java.engine.core.MarioLevelModel;
...
public class LevelGenerator implements MarioLevelGenerator {
    ...
    private void placeInterestingArrangement(MarioLevelModel model, int x, int y, int width) {
        for (int i = 0; i < width / 2; i++) {
            char type = MarioLevelModel.getBumpableTiles()[this.rnd.nextInt(
                MarioLevelModel.getBumpableTiles().length
            )];
        ...
        if (y > 4 && this.rnd.nextDouble() < 0.25) {
            this.placeInterestingArrangement(
                model,
                x + width / 4,
                y - 3 - this.rnd.nextInt(3),
                width / 2
            );
        }
    }
    ...
    private void placeEnemy(MarioLevelModel model, int x1, int x2, int y) {
        ...
        char enemyType = MarioLevelModel.getEnemyCharacters(false)[this.rnd.nextInt(
            MarioLevelModel.getEnemyCharacters(false).length
        )];
        ...
        for (int i = 0; i < length; i++) {
            if (model.getBlock(xStart + i, y) == MarioLevelModel.EMPTY) model.setBlock(
                xStart + i,
                y,
                enemyType
            );
    ...
    public String getGeneratedLevel(MarioLevelModel model, MarioTimer timer) {
        ...
        for (int x = 0; x < model.getWidth(); x++) {
            if (
                ...
                gapLength == 0 ||
                x < GROUND_PADDING ||
                x > model.getWidth() - 1 - GROUND_PADDING
            ) {
        ...
        for (int i = 0; i < groundArea.size() / 2; i++) {
            ...
            if (groundLength > GROUND_PIPE_LENGTH && this.rnd.nextDouble() < PIPE_PROB) {
                ...
                this.placePipe(
                    ...
                    x,
                    model.getHeight() - 3,
                    this.rnd.nextInt((PIPE_HEIGHT * 2) / 3) + PIPE_HEIGHT / 3
                );
        ...
        for (int i = 0; i < newAreas.size() / 2; i++) {
            ...
            groundArea.add(model.getHeight() - 3);
            if (
                groundLength > GROUND_COLLECTIBLE_LENGTH && this.rnd.nextDouble() < COLLECTIBLE_PROB
            ) {
        ...
        for (int i = 1; i < groundArea.size() / 4; i++) {
            ...
            if (groundLength > GROUND_ENEMY_LENGTH) {
                this.placeEnemy(
                    model,
                    groundArea.get(4 * i),
                    groundArea.get(4 * i + 2),
                    groundArea.get(4 * i + 1)
                );
            }
        }
