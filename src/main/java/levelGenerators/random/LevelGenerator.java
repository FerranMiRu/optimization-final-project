package main.java.levelGenerators.random;
...
public class LevelGenerator implements MarioLevelGenerator {
    ...
    public String getGeneratedLevel(MarioLevelModel model, MarioTimer timer) {
                ...
                if (y > GROUND_Y_LOCATION) {
                    ...
                    if (random.nextDouble() < OBSTACLES_PROB) {
                        ...
                        model.setBlock(
                            x,
                            y,
                            MarioLevelModel.getEnemyCharacters()[random.nextInt(
                                MarioLevelModel.getEnemyCharacters().length
                            )]
                        );
                    }
                    ...
                    if (random.nextDouble() < COLLECTIBLE_PROB) {
                        model.setBlock(
                            x,
                            y,
                            MarioLevelModel.getCollectablesTiles()[random.nextInt(
                                MarioLevelModel.getCollectablesTiles().length
                            )]
                        );
                    }
                }
        ...
        model.setRectangle(
            model.getWidth() - 1 - FLOOR_PADDING,
            14,
            FLOOR_PADDING,
            2,
            MarioLevelModel.GROUND
        );
