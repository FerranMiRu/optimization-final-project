package main.java.levelGenerators.benWeber;
...
public class LevelGenerator implements MarioLevelGenerator {
    ...
    private void placeBlock(MarioLevelModel model, int x, int y) {
        ...
        if (rand.nextDouble() < CHANCE_BLOCK_ENEMY) {
            char t = MarioLevelModel.getEnemyCharacters(false)[this.rand.nextInt(
    ...
    public String getGeneratedLevel(MarioLevelModel model, MarioTimer timer) {
        ...
        int lastY =
            GROUND_MAX_HEIGHT +
            (int) (rand.nextDouble() * (model.getHeight() - 1 - GROUND_MAX_HEIGHT));
        int y = lastY;
        int nextY = y;
